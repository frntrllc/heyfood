...
for target in \
  ...
  x86_64-apple-darwin \
  x86_64-unknown-linux-gnu; do
  "$ROOT/scripts/release/package.sh" "$ROOT/install.sh" 0.6.1 "$target" "$distribution"
done
"$ROOT/scripts/release/checksums.sh" "$distribution" 0.6.1
"$ROOT/scripts/release/verify-assets.sh" "$distribution" 0.6.1
[[ "$(wc -l <"$distribution/SHA256SUMS" | tr -d '[:space:]')" -eq 4 ]] ||
  fail "the release manifest must bind exactly four archives"

windows_asset="$distribution/heyfood-v0.6.1-x86_64-pc-windows-msvc.zip"
touch "$windows_asset"
if "$ROOT/scripts/release/checksums.sh" "$distribution" 0.6.1 >/dev/null 2>&1; then
  fail "checksum generation must reject a Windows v0.6.1 asset"
fi
if "$ROOT/scripts/release/verify-assets.sh" "$distribution" 0.6.1 >/dev/null 2>&1; then
  fail "complete-set verification must reject a Windows v0.6.1 asset"
fi
...
printf 'release scope contract: four v0.6.1 archives; Windows CI retained\n'
