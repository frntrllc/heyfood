...
test_default_release_is_v050() {
  ...
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.5.0/"
...
test_rejects_unsupported_or_unsafe_version_before_download() {
  ...
  assert_contains "$STDERR_LOG" "this installer supports heyfood 0.5.0"

  new_case version-injection
  export HEYFOOD_VERSION="0.5.0;touch-must-not-run"
  if run_installer; then
...
test_archive_shape_failure_preserves_existing_binary() {
  ...
  tar -czf "$ASSET_DIR/heyfood-v0.5.0-$(host_target).tar.gz" \
    -C "$CASE_DIR/payload" heyfood unexpected.txt
  printf '%s  %s\n' \
    "$(sha256_file "$ASSET_DIR/heyfood-v0.5.0-$(host_target).tar.gz")" \
    "heyfood-v0.5.0-$(host_target).tar.gz" >"$ASSET_DIR/SHA256SUMS"
  if run_installer; then
...
test_version_mismatch_preserves_existing_binary() {
  ...
  make_release 0.5.0 9.9.9
  write_existing_binary
  if run_installer; then
    fail "installer accepted an executable with the wrong version"
  fi
  assert_contains "$STDERR_LOG" "expected heyfood 0.5.0 before installation"
  assert_existing_binary_untouched
}
