...
test_source_invariants() {
  ...
  assert_contains "$INSTALLER" 'SUPPORTED_VERSION="0.7.1"'
  assert_contains "$INSTALLER" 'NATIVE_STATE_RELEASE_VERSION="0.7.1"'
...
test_exact_native_install() {
  new_case exact
  make_release 0.7.1 0.7.1 native
  export HEYFOOD_VERSION=0.7.1
  run_installer

  [[ -x "$BIN_DIR/heyfood" ]] || fail "native executable was not installed"
  [[ "$("$BIN_DIR/heyfood" --version)" == "heyfood 0.7.1" ]] ||
    fail "installed executable did not report the exact release version"
  assert_contains "$STDOUT_LOG" "Installed heyfood 0.7.1 at $BIN_DIR/heyfood"
  ...
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.7.1/SHA256SUMS"
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.7.1/heyfood-v0.7.1-$(host_target).tar.gz"
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.7.1/heyfood-v0.7.1-native-state.json"
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.7.1/heyfood-installer-v0.7.1-$(host_target).tar.gz"
}
...
test_default_release_is_current() {
  ...
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.7.1/"
...
test_rejects_unsupported_or_unsafe_version_before_download() {
  ...
  assert_contains "$STDERR_LOG" "this installer supports heyfood 0.7.1"

  new_case version-injection
  export HEYFOOD_VERSION="0.7.1;touch-must-not-run"
  if run_installer; then
...
test_migrates_known_legacy_pipx_symlink() {
  ...
  [[ "$("$BIN_DIR/heyfood" --version)" == "heyfood 0.7.1" ]] ||
    fail "legacy pipx migration did not install v0.7.1"
...
test_failed_install_preserves_known_legacy_pipx_symlink() {
  ...
  make_release 0.7.1 0.7.1 native
  make_legacy_pipx_link
  printf 'corruption\n' >>"$ASSET_DIR/heyfood-v0.7.1-$(host_target).tar.gz"
  if run_installer; then
...
#!/usr/bin/env bash
if [[ "${1:-}" == "--version" ]]; then
  printf 'heyfood 0.7.1\n'
  exit 0
fi
printf 'installed v0.7.1 sentinel\n'
EOF
  chmod 0755 "$BIN_DIR/heyfood"
}
...
test_archive_shape_failure_preserves_existing_binary() {
  ...
  tar -czf "$ASSET_DIR/heyfood-v0.7.1-$(host_target).tar.gz" \
    -C "$CASE_DIR/payload" heyfood unexpected.txt
  refresh_native_checksums 0.7.1
  if run_installer; then
...
test_version_mismatch_preserves_existing_binary() {
  ...
  make_release 0.7.1 9.9.9 native
  write_existing_binary
  if run_installer; then
    fail "installer accepted an executable with the wrong version"
  fi
  assert_contains "$STDERR_LOG" "expected heyfood 0.7.1 before installation"
  assert_existing_binary_untouched
}
...
test_current_installer_refuses_v062_before_download_and_preserves_state() {
  ...
  if run_installer; then
    fail "the current v0.7.1 installer accepted a v0.6.2 request"
  fi

  [[ ! -e "$DOWNLOAD_LOG" ]] ||
    fail "the current installer downloaded before rejecting the v0.6.2 request"
  assert_contains "$STDERR_LOG" \
    "this installer supports heyfood 0.7.1; requested 0.6.2"
  [[ "$("$BIN_DIR/heyfood" --version)" == "heyfood 0.7.1" ]] ||
    fail "the rejected v0.6.2 request replaced the installed v0.7.1 executable"
  [[ "$(sha256_file "$BIN_DIR/heyfood")" == "$binary_digest" ]] ||
    fail "the rejected v0.6.2 request changed the installed v0.7.1 bytes"
  [[ "$(sha256_file \
    "$HEYFOOD_STATE_DIR/data/compatibility/native-state-floor.v1.json")" == \
    "$floor_digest" ]] ||
...
test_verified_native_state_install_and_compatible_reinstall() {
  ...
  assert_contains "$DOWNLOAD_LOG" "heyfood-v0.7.1-native-state.json"
  assert_contains "$DOWNLOAD_LOG" "heyfood-installer-v0.7.1-$(host_target).tar.gz"
...
test_pre_native_state_and_disagreeing_metadata_preserve_existing_binary() {
  ...
  use_native_state_installer
  make_release 0.7.1 0.7.1 native exact 1 1
  write_existing_binary
  if run_installer; then
  ...
  use_native_state_installer
  make_release 0.7.1 0.7.1 native exact 1 2
  write_existing_binary
  if run_installer; then
...
test_structurally_invalid_candidate_manifests_preserve_existing_binary() {
  ...
  for variant in nested textual duplicate-top duplicate-nested wrong-schema invalid oversized; do
    ...
    use_native_state_installer
    make_release 0.7.1 0.7.1 native "$variant"
    write_existing_binary
    if run_installer; then
...
test_invalid_or_oversized_declarations_preserve_existing_binary() {
  ...
  write_existing_binary
  printf '%s' \
    '{"binary_version":"0.7.1","binary_version":"0.7.1","maximum_native_state_version":2,"native_state_capabilities":["household-account-slot-v1","household-lifecycle-lock-v1","household-migration-guard-v1","household-teardown-journal-v1"],"schema_version":1}' \
    >"$ASSET_DIR/heyfood-v0.7.1-native-state.json"
  refresh_native_checksums 0.7.1
  if run_installer; then
...
test_missing_or_failing_verifier_preserves_existing_binary() {
  ...
  make_release 0.7.1 0.7.1 native
  write_existing_binary
  rm "$ASSET_DIR/heyfood-v0.7.1-native-state.json"
  if run_installer; then
  ...
  make_release 0.7.1 0.7.1 native
  write_existing_binary
  rm "$ASSET_DIR/heyfood-installer-v0.7.1-$(host_target).tar.gz"
  if run_installer; then
  ...
  use_native_state_installer
  make_release 0.7.1 0.7.1 native exact 2 2 reject
  write_existing_binary
  if run_installer; then
...
test_interruption_before_replacement_preserves_existing_binary() {
  ...
  use_native_state_installer
  make_release 0.7.1 0.7.1 native exact 2 2 interrupt
  write_existing_binary
  if run_installer; then
