...
test_source_invariants() {
  ...
  assert_contains "$INSTALLER" 'SUPPORTED_VERSION="0.6.2"'
...
test_exact_native_install() {
  ...
  [[ "$("$BIN_DIR/heyfood" --version)" == "heyfood 0.6.2" ]] ||
    fail "installed executable did not report the exact release version"
  assert_contains "$STDOUT_LOG" "Installed heyfood 0.6.2 at $BIN_DIR/heyfood"
  ...
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.6.2/SHA256SUMS"
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.6.2/heyfood-v0.6.2-$(host_target).tar.gz"
}
...
test_default_release_is_v060() {
  ...
  assert_contains "$DOWNLOAD_LOG" "/releases/download/v0.6.2/"
...
test_rejects_unsupported_or_unsafe_version_before_download() {
  ...
  assert_contains "$STDERR_LOG" "this installer supports heyfood 0.6.2"

  new_case version-injection
  export HEYFOOD_VERSION="0.6.2;touch-must-not-run"
  if run_installer; then
...
test_migrates_known_legacy_pipx_symlink() {
  ...
  [[ "$("$BIN_DIR/heyfood" --version)" == "heyfood 0.6.2" ]] ||
    fail "legacy pipx migration did not install v0.6.2"
...
test_failed_install_preserves_known_legacy_pipx_symlink() {
  ...
  make_release 0.6.2
  make_legacy_pipx_link
  printf 'corruption\n' >>"$ASSET_DIR/heyfood-v0.6.2-$(host_target).tar.gz"
  if run_installer; then
...
test_archive_shape_failure_preserves_existing_binary() {
  ...
  tar -czf "$ASSET_DIR/heyfood-v0.6.2-$(host_target).tar.gz" \
    -C "$CASE_DIR/payload" heyfood unexpected.txt
  printf '%s  %s\n' \
    "$(sha256_file "$ASSET_DIR/heyfood-v0.6.2-$(host_target).tar.gz")" \
    "heyfood-v0.6.2-$(host_target).tar.gz" >"$ASSET_DIR/SHA256SUMS"
  if run_installer; then
...
test_version_mismatch_preserves_existing_binary() {
  ...
  make_release 0.6.2 9.9.9
  write_existing_binary
  if run_installer; then
    fail "installer accepted an executable with the wrong version"
  fi
  assert_contains "$STDERR_LOG" "expected heyfood 0.6.2 before installation"
  assert_existing_binary_untouched
}
