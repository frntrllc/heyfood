...
supported release is `0.9.0` for macOS and Linux. It includes offline
...
Install the supported native `v0.9.0` binary on macOS or Linux:
...
without `sudo` or shell-profile edits. The complete public v0.9.0 set is four
...
The v0.9.0 release can explain its exact installed automation
...
release exposes exactly ten typed read/discovery MCP tools, including two
disclosure-gated household reads and two capability-gated Diet reads. It
...
To disconnect this installation, revoke its hosted link, device, and app
session and clear both local credential stores:

```bash
heyfood logout
```

Logout is idempotent. Local credentials are cleared even when some hosted
cleanup cannot be confirmed; `--json` reports each teardown step and whether
its outcome is uncertain without exposing authorization material.
...
The supported release includes a bounded, read-only Diet guide. `heyfood diet`
and `heyfood diet list` browse the 22 guides currently served by hello.food;
...
The bounded `v0.9.0` release provides four native product archives and four
...
complete twelve-stage showcase are future work rather than `v0.9.0` release
gates.
...
included in `v0.9.0`. Human progress labels stay separate from backend tool
...
Health integrations are explicitly deferred from the supported `v0.9.0`
...
showcase and real-hardware voice qualification remain post-`v0.9.0`
