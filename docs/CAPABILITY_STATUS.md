...
| Native v0.5.0 | Previous supported release | The first qualified Rust recovery release; superseded by v0.6.0. |
| Native v0.6.0 | Supported | Adds exact installed self-description, opt-in receipt-bound skill/MCP setup for qualified Codex and Claude Code versions, and six bounded read/discovery MCP tools. |
| Hosted installer | Supported | Installs the checksum-verified native `v0.6.0` archive for macOS or Linux. |
...
| macOS native archives | Supported in v0.6.0 | Both architectures are Developer ID signed with hardened runtime, Apple notarized, and installed-artifact qualified. |
| Linux native archives | Supported in v0.6.0 | Both architectures are checksum verified, attested, and installed-artifact qualified. |
| Current native release | v0.6.0 | Supported on the four macOS/Linux targets listed above. |
...
| Grocery | `heyfood grocery` plus TUI confirmation cards | Read/export; direct preparation and confirm/cancel require exact controlling-terminal review phrases | Supported v0.6.0 command with human-only mutations |
...
| Interactive TUI | Launches authenticated chat and functional read/action panels | N/A | Supported in v0.6.0; packaged archives pass the bounded clean/returning-user, household Grocery, failure-safety, and 40/80/120-column matrix. |
...
| Agent self-description | `heyfood agent describe/guide/schema/doctor` | None | Supported in v0.6.0; offline, credential-free, bounded, and non-mutating. |
| Codex/Claude Agent Skill setup | `heyfood agent setup` / `agent uninstall` | N/A | Supported in v0.6.0 for the exact qualified host versions; dry-run is default, apply requires the reviewed plan digest, and setup registers MCP through host-owned commands. |
| Local read-only MCP | `heyfood mcp serve` | Existing authenticated read controllers | Supported in v0.6.0 with exactly six bounded read/discovery tools and no agent mutation surface. |
