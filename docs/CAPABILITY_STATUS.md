...
| Household context switching | Human TUI `/for me`, `/for <member>`, and `/for everyone`; committed scope survives restart | Ordinary hosted turns use the exact selected local declared-profile snapshot | Supported in v0.8.0 for humans; the agent handoff is limited to one exact disclosed additional member and cannot change scope |
