...
## `0.7.1` bounded release matrix

The `0.7.1` release gate remains intentionally narrower than the twelve-stage
...
prove the following `0.7.1` recovery-release gates:
...
requirements do not broaden the bounded `0.7.1` release matrix:
...
Ordinary Native CLI CI runs the bounded `0.7.1` behavior matrix from packaged
fixtures on macOS, Linux, and Windows. Windows execution is source
qualification for a future release, not a `v0.7.1` distribution claim. The protected
...
written. Installed captures verify the final alternate-screen, bracketed-paste,
and cursor state while tolerating ConPTY's valid interleaving. Rust CI's
companion `Internal PTY, signal, restoration vertical` verifies native signals
and canonical-mode restoration; terminal-guard tests verify body-error and
panic restoration. The Windows force-clean seam is available only through the
