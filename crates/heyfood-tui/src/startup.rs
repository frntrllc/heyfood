...
pub fn render_startup(frame: &mut Frame<'_>, area: Rect) {
    ...
    if show_banner {
        lines.extend(banner_lines(block_width));
        lines.push(Line::from(""));
    }
    for (action, key) in MENU {
        lines.push(pad_to(menu_line(action, key), block_width));
    }
    lines.push(Line::from(""));
    lines.push(pad_to(
        Line::from(Span::styled(announcement, theme::accent_emphasis())),
        block_width,
    ));
    lines.push(pad_to(
...
fn banner_lines(block_width: usize) -> Vec<Line<'static>> {
    let banner_width = BANNER
        .lines()
        .map(|line| line.chars().count())
        .max()
            ...
            let line = match accent {
                Some(&(_, start, length)) => {
                    ...
                    Line::from(vec![
                        ...
                        Span::raw(before),
                        Span::styled(middle, theme::accent()),
                        Span::raw(after),
                    ])
                }
                None => Line::from(vec![
...
fn menu_line(action: &'static str, key: &'static str) -> Line<'static> {
