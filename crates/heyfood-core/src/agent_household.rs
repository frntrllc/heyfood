...
const COMMIT_EVIDENCE_VERIFIER_CONTRACT: &[u8] = b"heyfood.household.commit-evidence.verifier.v1";

/// Persisted verifier for one repository-held commit-evidence capability.
///
/// The verifier is a one-way commitment to secret material derived by the
/// native repository from its durable account key and the exact tuple. The
/// proposal journal never stores that secret, and this type cannot issue a
/// proof from a caller-provided household-state DTO.
#[derive(Clone, Eq, PartialEq)]
pub struct HouseholdCommitEvidenceBindingV1 {
    verifier: CanonicalDigestV1,
    account: AccountId,
    proposal_ref: AgentHouseholdProposalIdV1,
    commit_id: CommitId,
}

/// Tuple-bound proof authority reconstructed only by the native repository
/// from its durable secret. A persisted binding contains only the verifier,
/// so callers that obtain a journal cannot recover this capability or mint a
/// repository proof.
#[doc(hidden)]
#[derive(Clone, Eq, PartialEq)]
pub struct HouseholdCommitEvidenceRepositoryAuthorityV1 {
    verifier: CanonicalDigestV1,
    account: AccountId,
    proposal_ref: AgentHouseholdProposalIdV1,
    commit_id: CommitId,
...
impl HouseholdCommitEvidenceRepositoryAuthorityV1 {
    /// Repository-adapter boundary. The secret must be derived from the
    /// durable account-bound evidence key after the exact reservation has
    /// been authenticated by the native repository.
    #[doc(hidden)]
    #[must_use]
    pub fn from_repository_secret(
        account: AccountId,
        proposal_ref: AgentHouseholdProposalIdV1,
        commit_id: CommitId,
        repository_secret: &[u8; 32],
    ) -> Self {
        let verifier =
            commit_evidence_verifier(repository_secret, &account, proposal_ref, commit_id);
        Self {
            verifier,
            account,
            proposal_ref,
    ...
    pub fn seal_applied_repository_observation(
        &self,
        ...
        Ok(AppliedHouseholdCommitProofV1 {
            repository_verifier: self.verifier,
            account: self.account.clone(),
            proposal_ref: self.proposal_ref,
            commit_id: self.commit_id,
            effect_fingerprint,
    ...
    pub fn seal_unapplied_repository_observation(
        &self,
        binding: &HouseholdCommitEvidenceBindingV1,
...
impl fmt::Debug for HouseholdCommitEvidenceRepositoryAuthorityV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("HouseholdCommitEvidenceRepositoryAuthorityV1")
            .finish_non_exhaustive()
    }
}
...
impl HouseholdCommitEvidenceBindingV1 {
    ...
    fn matches_repository_verifier(&self, repository_verifier: CanonicalDigestV1) -> bool {
        self.verifier == repository_verifier
    }
}

fn commit_evidence_verifier(
    repository_secret: &[u8; 32],
    account: &AccountId,
    proposal_ref: AgentHouseholdProposalIdV1,
    commit_id: CommitId,
    ...
    let mut hasher = Sha256::new();
    hasher.update(COMMIT_EVIDENCE_VERIFIER_CONTRACT);
    hasher.update(
    ...
    hasher.update(proposal_ref.as_uuid().as_bytes());
    hasher.update(commit_id.as_uuid().as_bytes());
    hasher.update(repository_secret);
    CanonicalDigestV1::from_bytes(hasher.finalize().into())
}
...
impl LocalHouseholdProposalAuthorityV1 {
    ...
    fn cancel_before_commit(&mut self) -> Result<(), AgentHouseholdContractErrorV1> {
    ...
    fn mark_committed(&mut self) -> Result<(), AgentHouseholdContractErrorV1> {
    ...
    fn mark_reconciliation_required(&mut self) -> Result<(), AgentHouseholdContractErrorV1> {
        if self.state != AgentHouseholdProposalStateV1::Committing {
            return Err(AgentHouseholdContractErrorV1::InvalidTransition);
        }
    ...
    fn reconcile_committed(&mut self) -> Result<(), AgentHouseholdContractErrorV1> {
...
pub struct AppliedHouseholdCommitProofV1 {
    repository_verifier: CanonicalDigestV1,
    account: AccountId,
    proposal_ref: AgentHouseholdProposalIdV1,
    commit_id: CommitId,
    effect_fingerprint: HouseholdEffectFingerprintV1,
...
impl fmt::Debug for AppliedHouseholdCommitProofV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AppliedHouseholdCommitProofV1")
...
impl LocalHouseholdProposalJournalV1 {
    pub fn new(
        ...
        if !matches!(
            ...
            AgentHouseholdProposalStateV1::Prepared
                | AgentHouseholdProposalStateV1::AwaitingLocalInput
        ) || authority.proposal_generation != GenerationId::INITIAL
            || authority.frozen.is_some()
        {
            return Err(AgentHouseholdContractErrorV1::InvalidJournal);
        }
        Ok(Self {
    ...
    pub fn reconcile_applied_commit(
        &mut self,
        expected: &LocalHouseholdProposalCasTokenV1,
        proof: &AppliedHouseholdCommitProofV1,
    ) -> Result<(), AgentHouseholdContractErrorV1> {
        ...
        let frozen = self
            ...
            .as_ref()
            .ok_or(AgentHouseholdContractErrorV1::MissingFrozenAuthority)?;
        let expected_resulting_revision = self
            .authority
            .binding
            .expected_household_revision
            .checked_next()
        ...
        if self.authority.binding.account != proof.account
            ...
            || !self
                ...
                .binding
                .commit_evidence_binding
                .matches_repository_verifier(proof.repository_verifier)
...
struct BindingWireV1 {
    ...
    expected_profile_revision: Option<ProfileRevision>,
    commit_id: CommitId,
    commit_evidence_verifier: CanonicalDigestV1,
    member_id: Option<MemberId>,
    previous_scope: HouseholdScope,
    originating_session_digest: CanonicalDigestV1,
    eligible_host_policy_digest: CanonicalDigestV1,
    ...
    fn from(value: &LocalHouseholdProposalJournalV1) -> Self {
        ...
        Self {
            ...
            binding: BindingWireV1 {
                ...
                commit_evidence_verifier: binding.commit_evidence_binding.verifier,
...
impl TryFrom<JournalWireV1> for LocalHouseholdProposalJournalV1 {
    ...
    fn try_from(value: JournalWireV1) -> Result<Self, Self::Error> {
        ...
        let commit_evidence_binding = HouseholdCommitEvidenceBindingV1 {
            verifier: value.binding.commit_evidence_verifier,
...
mod tests {
    ...
    fn binding_with_evidence(
        ...
        .map(|binding| (binding, repository_authority))
        .expect("binding")
    }
    ...
    fn durable_journal_cas_survives_restart_and_binds_the_applied_fingerprint() {
        let (primary_binding, repository_authority) = binding_with_evidence(
            AgentHouseholdOperationV1::Edit,
            GenerationId::new(4),
        ...
        let evidence_binding = repository_authority.binding();
        ...
        let mut transitioned_authority =
        ...
        transitioned_authority
            .freeze_for_review(&current, frozen.clone())
            .expect("bare transition for constructor rejection proof");
        assert_eq!(
            LocalHouseholdProposalJournalV1::new(transitioned_authority),
        ...
        let wrong_authority = HouseholdCommitEvidenceRepositoryAuthorityV1::from_repository_secret(
            AccountId::parse("phase0-proposal-account").expect("account"),
            proposal_ref,
            commit_id,
            &[0x99; 32],
        );
        let wrong_binding = wrong_authority.binding();
        let wrong_proof = wrong_authority
            .seal_applied_repository_observation(
                &wrong_binding,
                HouseholdEffectFingerprintV1::from_digest(CanonicalDigestV1::from_bytes([99; 32])),
                HouseholdRevision::new(8).expect("resulting revision"),
            )
            .expect("independent repository proof");
        assert_eq!(
            crash_recovered.reconcile_applied_commit(&committing_token, &wrong_proof),
            Err(AgentHouseholdContractErrorV1::AppliedCommitMismatch)
        );
        let skipped_revision_proof = repository_authority
            .seal_applied_repository_observation(
                &evidence_binding,
                effect_fingerprint,
                HouseholdRevision::new(9).expect("resulting revision"),
            )
            .expect("bound repository proof");
        assert_eq!(
            crash_recovered.reconcile_applied_commit(&committing_token, &skipped_revision_proof),
            Err(AgentHouseholdContractErrorV1::AppliedCommitMismatch)
        );
        let exact_proof = repository_authority
            .seal_applied_repository_observation(
                &evidence_binding,
                effect_fingerprint,
                HouseholdRevision::new(8).expect("resulting revision"),
            ...
            .expect("bound repository proof");
        crash_recovered
            .reconcile_applied_commit(&committing_token, &exact_proof)
    ...
    fn reconciliation_requires_bound_authority_to_prove_no_commit() {
        let (binding, repository_authority) = binding_with_evidence(
            AgentHouseholdOperationV1::Edit,
            GenerationId::new(4),
            ProfileRevision::new(2).ok(),
            Some(MemberId::new()),
        ...
        let wrong_binding = wrong_authority.binding();
        assert_eq!(
            wrong_authority.seal_unapplied_repository_observation(&evidence_binding, revision()),
            Err(AgentHouseholdContractErrorV1::AppliedCommitMismatch)
        );
        assert_eq!(
            repository_authority.seal_unapplied_repository_observation(&wrong_binding, revision()),
            Err(AgentHouseholdContractErrorV1::AppliedCommitMismatch)
        );
        let wrong_proof = wrong_authority
            .seal_unapplied_repository_observation(&wrong_binding, revision())
            .expect("independent repository proof");
        assert_eq!(
            journal.reconcile_unapplied_commit(&reconciliation, &wrong_proof),
            Err(AgentHouseholdContractErrorV1::AppliedCommitMismatch)
        );
        let exact = repository_authority
            .seal_unapplied_repository_observation(&evidence_binding, revision())
            .expect("bound repository proof");
        journal
