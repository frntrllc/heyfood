use heyfood_core::{
    ...
    Transcription, TranscriptionContractError, TranscriptionWire,
};
...
fn frozen_transcription_schema_bytes_and_runtime_limits_match() {
    ...
    let error_kinds = schema["x-heyfood-client-error-kinds"]
        .as_array()
        .unwrap()
        .iter()
        .map(|value| value.as_str().unwrap())
        .collect::<Vec<_>>();
    assert_eq!(error_kinds, TRANSCRIPTION_CLIENT_ERROR_KINDS);
