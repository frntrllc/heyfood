...
No Grocery Phase A wire artifact is frozen here. PR #107 remains provisional
until the corrected implementation is reviewed and merged, Production moves
from `095` to `096`, and the live backend advertises a proven `grocery: "v1"`
capability. Until those gates complete, the importer intentionally knows only
the merged C3/C4 inputs and cannot turn a PR head into an authoritative DTO.
